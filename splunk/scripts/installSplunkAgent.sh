...
#KDC: mount 192.168.61.132:\export/utilities-kdcprd/pkgs on /export/pkgs
...
collectd() {
    ...
    METHOD_STOCK="/export/pkgs/splunk/collectdsvc_stock.sh"
    METHOD_LOCAL="/opt/collectd/svc/collectdsvc.sh"
    CRON_STOCK="/opt/collectd/svc/cron-spkcollectd.sh"
...
#        done
        ...
        if cat "$CRON_DIR"/root | grep -q "cron-spkcollectd.sh" ; then
            ...
            sleep 2
            echo "0 0 * * * /opt/collectd/svc/cron-spkcollectd.sh> /dev/null 2>&1" >> $CRON_DIR/root
        fi
            # Startup
...
addoraclepermitcron() {
    ...
    if cat "$CRON_DIR"/root | grep -q "assertGroupPermits.sh" ; then
        ...
        sleep 2
        echo "0 0 * * * /root/splunk-scripts/assertGroupPermits.sh> /dev/null 2>&1" >> $CRON_DIR/root
    fi
...
addwlscron() {
    ...
    if cat "$CRON_DIR"/root | grep -q "splunkWlsMinute.sh" ; then
        ...
        sleep 2
        echo "* * * * * /root/splunk-scripts/splunkWlsMinute.sh> /dev/null 2>&1" >> $CRON_DIR/root
        sleep 2
    fi
    ...
    if cat "$CRON_DIR"/root | grep -q "splunkWlsHourly.sh" ; then
        ...
        sleep 2
        echo "0 * * * * /root/splunk-scripts/splunkWlsHourly.sh> /dev/null 2>&1" >> $CRON_DIR/root
    fi
    ...
    if [ ! -f /root/splunk-scripts/splunkWlsDaily.sh ] ; then
        ...
        (
            echo "#!/usr/bin/bash"
            echo "su - oracle -c '/opt/splunkforwarder/etc/apps/wls_admin_*/bin/runWlstScriptsDaily.sh'"
    ...
    if cat "$CRON_DIR"/root | grep -q "splunkWlsDaily.sh" ; then
        ...
        sleep 2
        echo "0 1 * * * /root/splunk-scripts/splunkWlsDaily.sh> /dev/null 2>&1" >> $CRON_DIR/root
    fi
...
#####
    ...
    if cat "$CRON_DIR"/root | grep -q "assertSplunkPermits.sh" ; then
        ...
        sleep 2
        echo "0 0 * * * /root/splunk-scripts/assertSplunkPermits.sh> /dev/null 2>&1" >> $CRON_DIR/root
    fi
